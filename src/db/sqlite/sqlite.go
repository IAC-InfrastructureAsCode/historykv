...
func New(filePath string, limit int, consulPrefix string) *DB {
	if filePath == "" {
		filePath = "/var/historykv.db"
	}
	...
	if err != nil {
		log.Fatalf("Couldn't connect to SQLite DB. File: %s\n", filePath)
	}
...
func (d *DB) CreateTable() {
	...
	historySTMT, errHistorySTMT := d.DB.Prepare(`CREATE TABLE IF NOT EXISTS kv_history(id INTEGER PRIMARY KEY AUTOINCREMENT, key TEXT NOT NULL, value BLOB, by TEXT NOT NULL, time INTEGER NOT NULL)`)

	if errHistorySTMT == nil {
		_, errHistorySTMT = historySTMT.Exec()
		if errHistorySTMT != nil {
			log.Fatalln("Error on creating User Table:", errHistorySTMT)
		}
	} else {
		log.Fatalln("Error on creating User Table:", errHistorySTMT)
	}

	userSTMT, errUserSTMT := d.DB.Prepare(`CREATE TABLE IF NOT EXISTS kv_user(id INTEGER PRIMARY KEY AUTOINCREMENT, user TEXT UNIQUE NOT NULL, password TEXT NOT NULL, token TEXT)`)
	...
	// Create index too!
	historyIndexSTMT, errHistoryIndexSTMT := d.DB.Prepare(`CREATE INDEX IF NOT EXISTS idx_kv_history_key_id ON kv_history(key, id DESC)`)

	if errHistoryIndexSTMT == nil {
		_, errHistoryIndexSTMT = historyIndexSTMT.Exec()
		if errHistoryIndexSTMT != nil {
			log.Println("Error on creating History Index:", errHistoryIndexSTMT)
